/*******************************************************************************
 * @jsname:billSignUp_controller.js
 * @author:WangDongping
 * @date:2017-08-18
 * @use:电子发票登记.js
 ******************************************************************************/
...
ngApp.controller('billScanController',['$scope','$http','$timeout','$sce',function($scope,$http,$timeout,$sce){
    ...
    $scope.submit = function(){
        if($scope.billList[0].bill.substring(0,2) != "BX" && $scope.billList[0].bill.substring(0,2) != "HX") {
            Biostime.common.toastMessage("请确信输入的第1行是单据号，第2行以后是电子发票号。");
            return;
        }
...
function submitBill($scope, $http, $sce){
    Biostime.common.showLoading();
    var url = BASICURL + "dealerPlatform/bxtracking/updateInvoiceElec.do";
    var list=[];
    for(var i=0; i<$scope.billList.length; i++){
